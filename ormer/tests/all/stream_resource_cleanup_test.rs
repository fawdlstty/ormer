...
async fn test_stream_pollution_on_error_impl(config: &crate::_test_common::DbConfig) {
...
async fn test_stream_early_termination_cleanup_impl(config: &crate::_test_common::DbConfig) {
    let db = crate::_test_common::create_db_connection(config).await.unwrap();
...
/// 测试多次连续流式查询，验证连接管理正确
async fn test_multiple_consecutive_streams_impl(config: &crate::_test_common::DbConfig) {
    let db = crate::_test_common::create_db_connection(config).await.unwrap();
...
/// 测试在连接池中使用流式查询
async fn test_stream_with_pool_cleanup_impl(config: &crate::_test_common::DbConfig) {
    // Turso后端连接池最大连接数必须为1
    #[cfg(feature = "sqlite")]
