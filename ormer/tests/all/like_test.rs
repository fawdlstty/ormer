...
async fn test_like_sql_impl(config: &crate::_test_common::DbConfig) {
    let _config = config;
...
async fn test_contains_sql_impl(config: &crate::_test_common::DbConfig) {
    let _config = config;
    let sql = ormer::Select::<TestUser>::new()
...
async fn test_starts_with_sql_impl(config: &crate::_test_common::DbConfig) {
    let _config = config;
    let sql = ormer::Select::<TestUser>::new()
...
async fn test_ends_with_sql_impl(config: &crate::_test_common::DbConfig) {
    let _config = config;
    let sql = ormer::Select::<TestUser>::new()
...
async fn test_like_e2e_sqlite() -> Result<(), Box<dyn std::error::Error>> {
    let config = crate::_test_common::get_sqlite_config();
    let db = crate::_test_common::create_db_connection(&config[0]).await?;
