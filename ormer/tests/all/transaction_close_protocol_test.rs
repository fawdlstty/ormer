...
async fn explicit_close_rolls_back() -> Result<(), Box<dyn std::error::Error>> {
    let db = crate::_test_common::create_db_connection(&crate::_test_common::sqlite_config()).await?;
    crate::_test_common::prepare_table::<CloseUser>(&db).await?;
    ...
    crate::_test_common::clean_table::<CloseUser>(&db).await?;
    Ok(())
}
...
async fn drop_rolls_back_active_transaction() -> Result<(), Box<dyn std::error::Error>> {
    let db = crate::_test_common::create_db_connection(&crate::_test_common::sqlite_config()).await?;
    crate::_test_common::prepare_table::<DropUser>(&db).await?;
    ...
    crate::_test_common::clean_table::<DropUser>(&db).await?;
    Ok(())
}
