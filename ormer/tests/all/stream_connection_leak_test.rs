...
/// 测试流式查询后连接是否正确释放
async fn test_stream_connection_release_impl(config: &crate::_test_common::DbConfig) {
...
/// 测试提前终止流时连接是否正确释放
async fn test_stream_early_termination_impl(config: &crate::_test_common::DbConfig) {
    let db = crate::_test_common::create_db_connection(config).await.unwrap();
...
async fn test_stream_in_transaction_release_impl(config: &crate::_test_common::DbConfig) {
    let db = crate::_test_common::create_db_connection(config).await.unwrap();
...
async fn test_stream_connection_release_mysql() {
    let config = crate::_test_common::mysql_config();
    test_stream_connection_release_impl(&config).await;
}
...
#[cfg(feature = "mysql")]
async fn test_stream_unsupported_in_transaction_mysql(config: &crate::_test_common::DbConfig) {
