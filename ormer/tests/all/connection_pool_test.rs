...
mod connection_pool_tests {
    ...
    async fn test_pool_creation_turso() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let _ = crate::_test_common::create_db_connection(&config).await?;

        // 创建连接池，min=0 表示创建时不建立连接
        // SQLite 建议使用单连接池（max_size=1）以避免并发写入冲突
        // 如需并发支持，可考虑启用 MVCC 模式（PRAGMA journal_mode = 'mvcc'）
        let pool = crate::_test_common::sqlite_pool().await?;

        // 从池中获取连接（此时才会真正创建连接）
        let conn = pool.get().await?;

        // 验证连接可以使用
        crate::_test_common::prepare_pooled_table::<PoolTestUser>(&conn).await?;

        // 清理测试表
        crate::_test_common::clean_pooled_table::<PoolTestUser>(&conn).await?;
    ...
    async fn test_pool_range_config_turso() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let pool1 = crate::_test_common::sqlite_pool().await?;

        let pool2 = crate::_test_common::sqlite_pool().await?;

        // 验证两个池都可以正常工作
        let conn1 = pool1.get().await?;
        crate::_test_common::prepare_pooled_table::<PoolTestUser>(&conn1).await?;

        let conn2 = pool2.get().await?;
        crate::_test_common::prepare_pooled_table::<PoolTestUser>(&conn2).await?;

        // 清理测试表 - 使用各自连接删除表（使用不同表名避免冲突）
        crate::_test_common::clean_pooled_table::<PoolTestUser>(&conn1).await?;
        crate::_test_common::clean_pooled_table::<PoolTestUser>(&conn2).await?;
